...
class robot:
    ...
    def sense(self, add_noise = 0): #do not change the name of this function
        ...
        for i in range(len(landmarks)):
            theta = atan2(landmarks[i][0] - self.y, landmarks[i][1] - self.x)
            theta = theta - self.orientation
            
            if add_noise:
                theta = theta + random.gauss(0, self.bearing_noise)
        ...
        return Z #Leave this line here. Return vector Z of (e.g. 4) bearings.
...
def particle_filter(motions, measurements, N=5000): # I know it's tempting, but don't change N!
    # --------
    #
    # Make particles
    # 
